...
- `display/` — the info/metadata panel (`InfoDisplay.tsx`, `MetadataCard.tsx`) and JSON import/export (`ImportExport.tsx`, generic `ImportExportButton<T>` that round-trips a context setter through file input / Blob download) and the button bar (`ButtonsBar.tsx`) — Inspect Keys toggle, Search toggle, Reset Pressed Keys, and (when the loaded diagram declares `modes`) a mode `<select>` that sets `activeMode` in `KeyboardUIContext`, filtering which shortcuts `Keyboard.tsx`/`SearchBar.tsx` show. `activeMode` is a distinct concept from `isInspectMode` — one picks a shortcut-mode filter, the other toggles the editing UI.
